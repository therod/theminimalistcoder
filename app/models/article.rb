class Article
  ...
  def next_article
    index = Article.all.find_index { |p| p.date == date } - 1
    if index == -1
      puts "No next article"
    else
      Article.find(index)
    end
  end
  ...
  class << self
    def all
      @@articles ||= Dir.glob(Rails.root + "app/data/articles/*.md").map do |filename|
    ...
    def where(conditions = {})
      ...
      all.select do |article|
        conditions.all? { |key, value| article.send(key) == value }
      end
    end
